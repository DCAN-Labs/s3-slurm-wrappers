...
template_pscalar="${run_folder}/template.pscalar.nii"
roi_label="${run_folder}/template.txt"
...
for subject in "${subjects[@]}"; do
	sed -e "s|SUBJECTID|${subject}|g" -e "s|WORKDIR|${work_dir}|g" -e "s|DATADIR|${data_dir}|g" -e "s|RUNDIR|${run_folder}|g" -e "s|OUTDIR|${out_dir}|g" -e "s|PSCALAR|${template_pscalar}|g" -e "s|ROILABEL|${roi_label}|g" ${run_folder}/${reliability_template} > ${reliability_folder}/run${k}
	k=$((k+1))
done
